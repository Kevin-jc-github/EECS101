...
int plot_logical_point2(float x_bar, float y_bar,unsigned char image[][COLS]);
...
{
	int				i;
	float			t, x, x_bar,x_bar_prime,y_bar_prime,y_bar,y, z,z_bar,xprime, yprime;
	float 			fprime = 1;
	FILE			*fp;
	char			 *ofile;
	...
	for (t = 0.01; t < 10000.01; t += .01)  //change the t range
	{
		...
		y_bar = -1 + t * 1;
		z = -1; /* when z = -1*/
		z_bar = -1; /* when z_bar = -1*/

		/* Modify these according to the projection */
		xprime = fprime*x/z;			/* this is an perspective projection */
		yprime = fprime*y/z;			

		x_bar_prime = fprime*x_bar/z_bar;
		y_bar_prime = fprime*y_bar/z_bar;

		plot_logical_point(xprime, yprime, image);
		plot_logical_point2(x_bar, y_bar, image);
	}
	//_CRT_SECURE_NO_WARNINGS
	/* Create a header */ 
	header(ROWS, COLS, head);

	ofile = "hw1-question2-pers1.ras";// this is the name of your output file, modify the name according to the requirements
...
int plot_logical_point2(float x_bar, float y_bar,unsigned char image[][COLS])
{
	...
	xc = COLS / ((float)LOGICAL_X_MAX - LOGICAL_X_MIN);
	yc = ROWS / ((float)LOGICAL_Y_MAX - LOGICAL_Y_MIN);
	nx = (x_bar - LOGICAL_X_MIN) * xc;
	ny = (y_bar - LOGICAL_Y_MIN) * yc;
	return plot_physical_point(nx, ny, image);
}
